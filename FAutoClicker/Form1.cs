...
namespace FAutoClicker
{
    public partial class MainWindow : Form
    {
        ...
        void LauguageCheck()
        {
            ...
            {
                label6.Text = "Product: FAutoClicker | Version: 1.0.0";
        ...
        void AutoClicker() //Основная функция автоклика
        {
            while (true) //Бесконечный цикл
            {
                if (clickerState) //Если кликер включен то
                {
                    ...
                    InputSimulator sim = new InputSimulator(); //Создание эмулятора
                    if (rb_Singl.Checked) //Если включен пункт одиночного нажатия
                    {
                        if (rb_Left.Checked) //Если включено нажатие левой кнопкой мыши
                            sim.Mouse.LeftButtonClick(); //Эмулирование одиночного клика левой кнопкой мыши
                        else //Если включено нажатие правой кнопкой мыши
                            sim.Mouse.RightButtonClick(); //Эмулирование одиночного клика правой кнопкой мыши
                    }
                    else //Если включен пункт двойного нажатия
                    {
                        if (rb_Left.Checked) //Если включено нажатие левой кнопкой мыши
                            sim.Mouse.LeftButtonDoubleClick(); //Эмулирование двойного клика левой кнопкой мыши
                        else //Если включено нажатие правой кнопкой мыши
                            sim.Mouse.RightButtonDoubleClick(); //Эмулирование двойного клика правой кнопкой мыши
                    }
